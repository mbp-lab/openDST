...
| `REACT_APP_FACE_DETECTION_MIN_CONFIDENCE` | Decimal (`0`–`1`) | `0.5` | Minimum MediaPipe face-detection score eligible for cropping. A higher value reduces low-confidence detections but can increase missed faces. |
| `REACT_APP_FACE_DETECTION_MIN_SUPPRESSION_THRESHOLD` | Decimal (`0`–`1`) | `0.3` | MediaPipe overlap/duplicate suppression threshold. It does not select the participant when several faces are detected. |
